  ...
  try {
    ...
    parentDetailsArray.forEach(({ details: parentDetails }) => {
      if (parentDetails && parentDetails.type) {
        parentDetailsMap[parentDetails.type] = parentDetails; // Use 'type' as the key
      }
    });
    ...
    for (const [key, value] of Object.entries(template.inputs)) {
      if (typeof value === "string" && value.startsWith("{{") && value.endsWith("}}")) {
        ...
        if (placeholder.startsWith("inputFields.")) {
          ...
          resolvedValue = inputFields[inputFieldKey] || null; // Fallback to null if undefined
        } else if (parentType && pathParts.length > 1 && pathParts[0] === "outputs") {
          // Check parent details for outputs
          const outputKey = pathParts[1]; // Extract "VpcId" from "outputs.VpcId"
        ...
        if (resolvedValue === undefined) {
          ...
          unresolvedPlaceholders.push({
            key,
            placeholder: value,
            reason: `No matching data found in parentDetails or inputFields.`,
          });
